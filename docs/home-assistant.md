...
| [`home/dot_local/bin/wrappers/hass-cli.ps1`](../home/dot_local/bin/wrappers/hass-cli.ps1)                                 | Escapes arguments for the binary, PowerShell                         |
...
| [`test/HassCliWrapper.Tests.ps1`](../test/HassCliWrapper.Tests.ps1)                                                       | Pester suite for the wrapper's argument passing, in CI               |
...
Same idea, spread over several files, because each Windows shell looks up a bare command name its own way.
...
The `.cmd` costs one thing at the call site, which the PowerShell file buys back. PowerShell quotes a native argument for the callee it sees, and for a `.cmd` that means cmd's convention: inner quotes cross bare. cmd hands the text to hass-cli.exe, whose own parser then eats them, so `--json '{"entity_id": "switch.x"}'` arrived as `{entity_id: switch.x}` and hass-cli reported a `JSONDecodeError` about property names — a message that reads as malformed JSON rather than as quoting, and cost two sessions before it was traced. [`hass-cli.ps1`](../home/dot_local/bin/wrappers/hass-cli.ps1) sets `$PSNativeCommandArgumentPassing = 'Standard'` for its own scope, which escapes those quotes as `\"`, and delegates to the `.cmd`: PowerShell resolves a `.ps1` ahead of a `.cmd`, so PowerShell callers get the quoting fix and everyone else reaches the `.cmd` unchanged. Credentials stay the `.cmd`'s job. [`test/HassCliWrapper.Tests.ps1`](../test/HassCliWrapper.Tests.ps1) models the chain with a stub rather than the real CLI, so it needs no instance.
