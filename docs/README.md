...
| [`git.md`](git.md)                   | Opinionated push/pull/merge defaults, commit signing backends per host, delta + KDiff3 wiring, helper scripts (`clean-safe`, `git j`, gist-backed `git unpicked` / `git amt`), per-OS quirks |
| [`gpg.md`](gpg.md)                   | Per-OS GPG pinentry wiring (Windows wrapper, Linux/macOS native, Termux popup), AI-agent bypass via `AI_AGENT`/`CLAUDE_CODE`                                                                 |
| [`m365.md`](m365.md)                 | CLI for Microsoft 365 (work-only, `ewn` hosts), one-time browser sign-in against a self-registered single-tenant Entra app, why the well-known first-party app IDs don't fit                 |
...
| [`mise.md`](mise.md)                 | mise install + activation per platform, Android-only global config, the Termux out-of-band hk/pkl/actionlint lint toolchain, version pinning, `mise bootstrap` vs chezmoi                    |
...
| [`ssh.md`](ssh.md)                   | Shared `Include ./*.conf` config, private host inventory repo + tag-driven behavior, Windows-native OpenSSH as the canonical client, WSL→Windows agent bridge, Android popup askpass         |
